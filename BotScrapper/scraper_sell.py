...
try:
    ...
    print("Connexion à la base BotScrappingSell réussie !")
except mysql.connector.Error as err:
    print(f"Erreur de connexion à la BDD: {err}")
    exit(1)

# --- Création/mise à jour de la table quotes_sell ---
create_table_query = """
CREATE TABLE IF NOT EXISTS quotes_sell (
...
try:
    ...
    connection.commit()
    print("Table quotes_sell prête ou déjà existante.")
except mysql.connector.Error as err:
...
def scrape_transactions():
    try:
        # On utilise l'option 'latest' et on filtrera les SELL ensuite
    ...
    # Filtrer uniquement les transactions de type "Sale"
    df = df[df['Transaction'].str.contains('Sale', case=False)]
    
    if df.empty:
        print("Aucune transaction SELL trouvée.")
        return []
...
def save_transactions_to_db(transactions):
    ...
    connection.commit()
    print(f"{count} transactions SELL insérées dans la base de données.")
...
def delete_old_unsaved_transactions():
    ...
    try:
        ...
        connection.commit()
        if nb_deleted > 0:
            print(f"{nb_deleted} transactions SELL non sauvegardées (plus de 10 min) supprimées.")
    except mysql.connector.Error as err:
...
if __name__ == "__main__":
    while True:
        print("Début du scraping des transactions SELL Finviz Insider Trading en se basant sur SEC Form 4 (<10 min)...")
        transactions = scrape_transactions()
        if transactions:
            save_transactions_to_db(transactions)
        else:
            print("Aucune transaction SELL trouvée dans les 10 dernières minutes ou parsing impossible.")
