...
try:
    ...
    print("Connexion à la base BotScrapping réussie !")
except mysql.connector.Error as err:
    print(f"Erreur de connexion à la BDD: {err}")
    exit(1)

# --- Création/mise à jour de la table quotes ---
create_table_query = """
...
try:
    ...
    connection.commit()
    print("Table quotes prête ou déjà existante.")
except mysql.connector.Error as err:
...
def save_transactions_to_db(transactions):
    ...
    connection.commit()
    print(f"{count} transactions insérées dans la base de données.")
...
def delete_old_unsaved_transactions():
    ...
    try:
        ...
        connection.commit()
        if nb_deleted > 0:
            print(f"{nb_deleted} transactions non sauvegardées (plus de 10 min) supprimées.")
    except mysql.connector.Error as err:
...
if __name__ == "__main__":
    while True:
        print("Début du scraping des transactions Finviz Insider Trading (tc=7) en se basant sur SEC Form 4 (<10 min)...")
        transactions = scrape_transactions()
        if transactions:
            save_transactions_to_db(transactions)
        else:
            print("Aucune transaction trouvée dans les 10 dernières minutes ou parsing impossible.")
